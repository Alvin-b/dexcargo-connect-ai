...
function StaffPage() {
  ...
  function openEdit(u: UserRow) {
    ...
    setPendingLocation(u.profile?.staff_location ?? "");
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Promote users to staff roles, assign their warehouse location, or revoke access.
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>All users {users.length ? `(${users.length})` : ""}</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              ...
              {users.map((u) => (
                <TableRow key={u.id}>
                  ...
                  <TableCell className="text-xs">{u.profile?.staff_location ?? "—"}</TableCell>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Edit roles — {editing?.email}</DialogTitle>
          </DialogHeader>
