...
function MarketingPage() {
  ...
  async function addImage(postId: string, prompt: string) {
    setBusyId(postId);
    try {
      await genImg({ data: { postId, prompt } });
  ...
  async function publishNow(postId: string) {
    ...
    try {
      ...
      if (r.ok) toast.success("Published");
      else toast.error(`Publish failed: ${(r.result as any).error}`);
      qc.invalidateQueries({ queryKey: ["posts"] });
    } catch (e: any) {
      toast.error(e.message);
    } finally {
  ...
  async function schedule(postId: string) {
    ...
    setBusyId(postId);
    try {
      await sched({ data: { postId, scheduledFor: new Date(scheduleAt).toISOString() } });
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Generate posts, attach AI images, schedule and publish to social platforms.
        </p>
      </div>
      <Card>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="col-span-2">
              ...
              <Input
                ...
                placeholder="e.g. New air freight rates from Guangzhou to Nairobi"
              />
            </div>
          </div>
        ...
        {posts?.map((p) => (
          <Card key={p.id}>
            <CardContent className="p-4 space-y-3">
              ...
              {p.image_url && (
                <img src={p.image_url} alt="" className="rounded-md max-h-72 object-cover w-full" />
              )}
              ...
              {p.external_post_id && (
                <div className="text-xs text-muted-foreground">
                  External ID: {p.external_post_id}
                </div>
              )}
              {p.status !== "published" && (
                <div className="flex flex-wrap gap-2 pt-2 border-t">
                  {!p.image_url && (
                    <Button
                      ...
                      variant="outline"
                      disabled={busyId === p.id}
                      onClick={() => addImage(p.id, p.content.slice(0, 200))}
                    >
                      <ImageIcon className="h-3.5 w-3.5 mr-1" />
                      {busyId === p.id ? "Generating…" : "Add AI image"}
                    </Button>
                  )}
                  ...
                  {scheduleId === p.id ? (
                    <div className="flex gap-2 items-center">
                      ...
                      <Button
                        size="sm"
                        disabled={busyId === p.id || !scheduleAt}
