...
import { quoteShipping } from "@/server/notify.functions";
...
export const Route = createFileRoute("/quote")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Calculate the cost of shipping your cargo from China by air, sea or express." },
    ],
  }),
});
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b">
        <div className="max-w-4xl mx-auto px-4 py-4 flex justify-between">
          <Link to="/" className="text-2xl font-bold text-primary">Dexcargo</Link>
      ...
      <main className="max-w-3xl mx-auto px-4 py-12">
        ...
        <Card><CardContent className="p-6 space-y-4">
          <div className="grid md:grid-cols-3 gap-3">
            {([
              ...
              <button key={m.v} type="button" onClick={() => setMode(m.v)}
                ...
                <m.icon className="h-5 w-5" /><span className="text-sm font-medium">{m.label}</span>
              </button>
            ))}
          </div>
          <div className="grid md:grid-cols-2 gap-3">
            ...
            {mode === "sea" && (
              <div><Label>Volume (CBM)</Label>
                <Input type="number" step="0.01" value={cbm} onChange={(e) => setCbm(e.target.value)} placeholder="e.g. 1.5" />
              </div>
            )}
          </div>
          ...
          {result && (
            <div className={`rounded-md p-4 ${result.ok ? "bg-primary/5 border border-primary/20" : "bg-destructive/5 border border-destructive/20"}`}>
              {result.ok ? (
                <>
                  <div className="text-sm text-muted-foreground">Estimated cost</div>
                  <div className="text-3xl font-bold">{result.currency} {result.cost.toLocaleString()}</div>
                  <div className="text-sm text-muted-foreground mt-1">{result.message}</div>
                  <p className="text-xs text-muted-foreground mt-3">Final cost may vary based on actual weight, volume and customs. Contact us on WhatsApp to confirm.</p>
                </>
              ) : (
